...
def safe_print(*args, **kwargs):
    ...
    should_print = True
    if CLEAN_CONSOLE_LOGS:
        allow_markers = ["🤖 Interviewer", "👤", "SUMMARY:", "Interviewer:", "Candidate:"]
        should_print = any(m in msg for m in allow_markers)
        if SHOW_RAG_IN_CONSOLE and not should_print:
            rag_markers = ["RAG_ADDED_SNIPPET", "RAG_USED_FOR_QUESTION", "[RAG ADD]", "[BG RAG]"]
            should_print = any(m in msg for m in rag_markers)
